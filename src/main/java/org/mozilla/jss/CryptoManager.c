...
int ConfigureOCSP(JNIEnv *env, jboolean ocspCheckingEnabled,
		jstring ocspResponderURL, jstring ocspResponderCertNickname) {
	...
	ocspResponderCertNickname_string = JSS_RefJString(env,
	...
	if (ocspResponderURL_string) {
		...
		if (ocspResponderCertNickname == NULL) {
			...
			cert = CERT_FindCertByNickname(certdb,
			...
			if (cert == NULL) {
				...
				cert = PK11_FindCertFromNickname(
						ocspResponderCertNickname_string, NULL);
			}
			if (cert == NULL) {
				JSS_throwMsg(env, GENERAL_SECURITY_EXCEPTION,
		...
		status = CERT_SetOCSPDefaultResponder(certdb, ocspResponderURL_string,
				ocspResponderCertNickname_string);
	...
	finish: JSS_DerefJString(env, ocspResponderURL, ocspResponderURL_string);
	JSS_DerefJString(env, ocspResponderCertNickname,
			ocspResponderCertNickname_string);
...
Java_org_mozilla_jss_CryptoManager_initializeAllNative(JNIEnv *env,
		jclass clazz, jstring configDir, jstring certPrefix, jstring keyPrefix,
		jstring secmodName, jboolean readOnly, jstring manuString,
		jstring libraryString, jstring tokString, jstring keyTokString,
		jstring slotString, jstring keySlotString, jstring fipsString,
		jstring fipsKeyString, jboolean ocspCheckingEnabled,
		jstring ocspResponderURL, jstring ocspResponderCertNickname,
		jboolean initializeJavaOnly, jboolean PKIXVerify, jboolean noCertDB,
		jboolean noModDB, jboolean forceOpen, jboolean noRootInit,
		jboolean optimizeSpace, jboolean PK11ThreadSafe, jboolean PK11Reload,
		jboolean noPK11Finalize, jboolean cooperate) {
	Java_org_mozilla_jss_CryptoManager_initializeAllNative2(env, clazz,
			configDir, certPrefix, keyPrefix, secmodName, readOnly, manuString,
			libraryString, tokString, keyTokString, slotString, keySlotString,
			fipsString, fipsKeyString, ocspCheckingEnabled, ocspResponderURL,
			ocspResponderCertNickname, JNI_FALSE, /*initializeJavaOnly*/
			PKIXVerify, noCertDB, noModDB, forceOpen, noRootInit, optimizeSpace,
			PK11ThreadSafe, PK11Reload, noPK11Finalize, cooperate);
}

JNIEXPORT void JNICALL
Java_org_mozilla_jss_CryptoManager_initializeAllNative2(JNIEnv *env,
		jclass clazz, jstring configDir, jstring certPrefix, jstring keyPrefix,
		jstring secmodName, jboolean readOnly, jstring manuString,
		jstring libraryString, jstring tokString, jstring keyTokString,
		jstring slotString, jstring keySlotString, jstring fipsString,
	...
	if (configDir == NULL || manuString == NULL || libraryString == NULL
			|| tokString == NULL || keyTokString == NULL || slotString == NULL
			|| keySlotString == NULL || fipsString == NULL
			|| fipsKeyString == NULL) {
	...
	PK11_ConfigurePKCS11(manuChars, libraryChars, tokChars, keyTokChars,
			slotChars, keySlotChars, fipsChars, fipsKeyChars, 0, /* minimum pin length */
			PR_FALSE /* password required */
			);

	szConfigDir = JSS_RefJString(env, configDir);
	if (certPrefix != NULL || keyPrefix != NULL || secmodName != NULL
			|| noCertDB || noModDB || forceOpen || noRootInit || optimizeSpace
			|| PK11ThreadSafe || PK11Reload || noPK11Finalize || cooperate) {
		/*
	 ...
	 */
	rv = ConfigureOCSP(env, ocspCheckingEnabled, ocspResponderURL,
			ocspResponderCertNickname);
	...
	finish: JSS_DerefJString(env, configDir, szConfigDir);
 ...
 *
 */
void JSS_setPasswordCallback(JNIEnv *env, jobject callback) {
	PR_ASSERT(env != NULL);
 ...
 */
JNIEXPORT void JNICALL
Java_org_mozilla_jss_CryptoManager_setNativePasswordCallback(JNIEnv *env,
...
getPWFromCallback(PK11SlotInfo *slot, PRBool retry, void *arg) {
	...
	if ((*JSS_javaVM)->AttachCurrentThread(JSS_javaVM, (void**) &env, NULL)
			!= 0) {
		PR_ASSERT(PR_FALSE);
		goto finish;
	}
	...
	if (retry) {
		getPWMethod = (*env)->GetMethodID(env, callbackClass,
	 ...
	 *****************************************/
	pwObject = (*env)->CallObjectMethod(env, callback, getPWMethod, pwcbInfo);
	...
	getByteCopyMethod = (*env)->GetMethodID(env, passwordClass,
	PW_GET_BYTE_COPY_NAME,
	PW_GET_BYTE_COPY_SIG);
	clearMethod = (*env)->GetMethodID(env, passwordClass,
	PW_CLEAR_NAME,
	PW_CLEAR_SIG);
 ...
 */
static jobject makePWCBInfo(JNIEnv *env, PK11SlotInfo *slot) {
	jclass infoClass;
	...
	constructor = (*env)->GetMethodID(env, infoClass,
 ...
 */
JNIEXPORT void JNICALL
Java_org_mozilla_jss_CryptoManager_putModulesInVector(JNIEnv *env, jobject this,
		jobject vector) {
	...
	addElement = (*env)->GetMethodID(env, vectorClass,
	 ...
	 ***************************************************/
	for (list = SECMOD_GetDefaultModuleList(); list != NULL; list =
			list->next) {
...
Java_org_mozilla_jss_CryptoManager_enableFIPS(JNIEnv *env, jclass clazz,
		jboolean fips) {
	char *name = NULL;
	...
	if (status != SECSuccess) {
		JSS_throwMsgPortErr(env, GENERAL_SECURITY_EXCEPTION,
 ...
 */
JNIEXPORT void JNICALL
Java_org_mozilla_jss_DatabaseCloser_closeDatabases(JNIEnv *env, jobject this) {
	NSS_Shutdown();
}
 ...
 */
JNIEXPORT void JNICALL
Java_org_mozilla_jss_CryptoManager_configureOCSPNative(JNIEnv *env,
		jobject this, jboolean ocspCheckingEnabled, jstring ocspResponderURL,
	...
	if (rv != SECSuccess) {
		JSS_throwMsgPrErr(env, GENERAL_SECURITY_EXCEPTION,
 ...
 */
JNIEXPORT void JNICALL
Java_org_mozilla_jss_CryptoManager_OCSPCacheSettingsNative(JNIEnv *env,
		jobject this, jint ocsp_cache_size, jint ocsp_min_cache_entry_duration,
		jint ocsp_max_cache_entry_duration) {
	SECStatus rv = SECFailure;

	rv = CERT_OCSPCacheSettings(ocsp_cache_size, ocsp_min_cache_entry_duration,
...
JNIEXPORT void JNICALL
Java_org_mozilla_jss_CryptoManager_setOCSPTimeoutNative(JNIEnv *env,
		jobject this, jint ocsp_timeout) {
	SECStatus rv = SECFailure;
...
Java_org_mozilla_jss_CryptoManager_getJSSMajorVersion(JNIEnv *env, jobject this) {
	return JSS_VMAJOR;
}

JNIEXPORT int JNICALL
Java_org_mozilla_jss_CryptoManager_getJSSMinorVersion(JNIEnv *env, jobject this) {
	return JSS_VMINOR;
}

JNIEXPORT int JNICALL
Java_org_mozilla_jss_CryptoManager_getJSSPatchVersion(JNIEnv *env, jobject this) {
	return JSS_VPATCH;
}
