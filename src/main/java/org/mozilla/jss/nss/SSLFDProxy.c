...
PRStatus JSS_NSS_getSSLClientCert(JNIEnv *env, jobject sslfd_proxy,
...
static PRStatus JSS_NSS_getEventArrayList(JNIEnv *env, jobject sslfd_proxy,
		const char *which, jobject *list) {
	jclass sslfdProxyClass;
...
PRStatus JSS_NSS_getSSLAlertReceivedList(JNIEnv *env, jobject sslfd_proxy,
		jobject *list) {
	return JSS_NSS_getEventArrayList(env, sslfd_proxy, "inboundAlerts", list);
}

PRStatus JSS_NSS_getSSLAlertSentList(JNIEnv *env, jobject sslfd_proxy,
		jobject *list) {
	return JSS_NSS_getEventArrayList(env, sslfd_proxy, "outboundAlerts", list);
}

PRStatus JSS_NSS_getGlobalRef(JNIEnv *env, jobject sslfd_proxy,
		jobject *global_ref) {
	PR_ASSERT(env != NULL && sslfd_proxy != NULL && global_ref != NULL);

	if (JSS_getPtrFromProxyOwner(env, sslfd_proxy, "globalRef",
			"L" GLOBAL_REF_PROXY_CLASS_NAME ";", (void**) global_ref)
			== PR_FAILURE || *global_ref == NULL) {
...
PRStatus JSS_NSS_addSSLAlert(JNIEnv *env, jobject sslfd_proxy, jobject list,
	...
	PR_ASSERT(
			env != NULL && sslfd_proxy != NULL && list != NULL
					&& alert != NULL);
	...
	event = (*env)->NewObject(env, eventClass, eventConstructor, sslfd_proxy,
			(int) alert->level, (int) alert->description);
	if (event == NULL) {
		return PR_FAILURE;
	}
...
void JSSL_SSLFDAlertReceivedCallback(const PRFileDesc *fd, void *arg,
		const SSLAlert *alert) {
	JNIEnv *env;
	...
	if (JSS_NSS_getSSLAlertReceivedList(env, sslfd_proxy, &list)
			!= PR_SUCCESS) {
		return;
	}
...
void JSSL_SSLFDAlertSentCallback(const PRFileDesc *fd, void *arg,
		const SSLAlert *alert) {
	JNIEnv *env;
	jobject sslfd_proxy = (jobject) arg;
	jobject list;
...
SECStatus JSSL_SSLFDCertSelectionCallback(void *arg, PRFileDesc *fd,
	...
	for (slotElement = slotList->head; slotElement;
			slotElement = slotElement->next) {
		privkey = PK11_FindPrivateKeyFromCert(slotElement->slot, cert,
				NULL /* pinarg */);
		if (privkey != NULL) {
			break;
		}
...
void JSSL_SSLFDHandshakeComplete(PRFileDesc *fd, void *client_data) {
	JNIEnv *env = NULL;
...
SECStatus JSSL_SSLFDAsyncCertAuthCallback(void *arg, PRFileDesc *fd,
	...
	(*env)->SetBooleanField(env, sslfd_proxy, needCertValidationField,
			JNI_TRUE);

	return SECWouldBlock;
}

SECStatus JSSL_SSLFDSyncCertAuthCallback(void *arg, PRFileDesc *fd,
		PRBool checkSig, PRBool isServer) {
	/* We know that arg is our GlobalRefProxy instance pointing to the
	 * SSLFDProxy class instance. This lets us ignore the isServer parameter,
	 * because we can infer that from our JSSEngine instance. Additionally,
...
SECStatus JSSL_SSLFDAsyncBadCertCallback(void *arg, PRFileDesc *fd) {
	/* We know that arg is our GlobalRefProxy instance pointing to the
	...
	badCertErrorField = (*env)->GetFieldID(env, sslfdProxyClass, "badCertError",
			"I");
	if (badCertErrorField == NULL) {
		return SECFailure;
	}

	(*env)->SetBooleanField(env, sslfd_proxy, needBadCertValidationField,
			JNI_TRUE);
	(*env)->SetIntField(env, sslfd_proxy, needBadCertValidationField,
			cert_error);

	return SECWouldBlock;
}

SECStatus JSSL_SSLFDSyncBadCertCallback(void *arg, PRFileDesc *fd) {
	/* We know that arg is our GlobalRefProxy instance pointing to the
	 * SSLFDProxy class instance. This lets us ignore the PRFileDesc
	 * parameter because we already have a reference to it via arg.
	 *
	...
	ret = (*env)->CallIntMethod(env, sslfd_proxy, badCertHandlerMethod,
			cert_error);
	if ((*env)->ExceptionOccurred(env) != NULL) {
		ret = PR_UNKNOWN_ERROR;
	}
