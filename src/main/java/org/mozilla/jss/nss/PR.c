...
Java_org_mozilla_jss_nss_PR_NewBufferPRFD(JNIEnv *env, jclass clazz,
	...
	if (peer_info != NULL
			&& !JSS_FromByteArray(env, peer_info, &real_peer_info,
					&peer_info_len)) {
		return result;
	}
...
Java_org_mozilla_jss_nss_PR_Read(JNIEnv *env, jclass clazz, jobject fd,
	...
	if (fd_type == PR_DESC_SOCKET_TCP || fd_type == PR_DESC_SOCKET_UDP
	...
	while (read_amount < amount) {
		this_read = PR_Read(real_fd, buffer + read_amount,
				amount - read_amount);
		if (this_read <= 0) {
			if (PR_GetError() == 0) {
				/* End of data */
...
Java_org_mozilla_jss_nss_PR_Recv(JNIEnv *env, jclass clazz, jobject fd,
	...
	PR_ASSERT(
			env != NULL && fd != NULL && amount >= 0 && flags >= 0
...
Java_org_mozilla_jss_nss_PR_Send(JNIEnv *env, jclass clazz, jobject fd,
	...
	PR_ASSERT(
			env != NULL && fd != NULL && buf != NULL && flags >= 0
					&& timeout >= 0 && timeout <= UINT32_MAX);
	PR_SetError(0, 0);
...
JNIEXPORT jbyteArray JNICALL
Java_org_mozilla_jss_nss_PR_ErrorToNameNative(JNIEnv *env, jclass clazz,
		jint error_code) {
	size_t error_size;
