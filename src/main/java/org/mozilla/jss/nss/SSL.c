...
jobject JSS_NewSSLPreliminaryChannelInfo(JNIEnv *env, jlong valuesSet,
	...
	resultClass = (*env)->FindClass(env,
			SSL_PRELIMINARY_CHANNEL_INFO_CLASS_NAME);
	if (resultClass == NULL) {
		ASSERT_OUTOFMEM(env);
		goto finish;
	}
...
Java_org_mozilla_jss_nss_SSL_ImportFDNative(JNIEnv *env, jclass clazz,
		jobject model, jobject fd) {
	...
	if (model != NULL
			&& (JSS_PR_getPRFileDesc(env, model, &real_model) != PR_SUCCESS
					|| real_model == NULL)) {
		return NULL;
	}

	if (fd != NULL
			&& (JSS_PR_getPRFileDesc(env, fd, &real_fd) != PR_SUCCESS
					|| real_fd == NULL)) {
		return NULL;
	}
...
JNIEXPORT int JNICALL
Java_org_mozilla_jss_nss_SSL_VersionRangeSetDefaultNative(JNIEnv *env,
		jclass clazz, jint variant_ssl, jint min_ssl, jint max_ssl) {
	SSLVersionRange vrange;
	...
	variant_ssl < 0 || variant_ssl >= JSSL_enums_size) {
		...
		snprintf(buf, 200,
				"SSL.VersionRangeSetDefaultNative(): for min=%d max=%d "
						...
						"size: %d", min_ssl, max_ssl, variant_ssl,
				JSSL_enums_size);
		JSSL_throwSSLSocketException(env, buf);
		return SECFailure;
	}
	...
	if (variant_ssl < 0 || variant_ssl >= JSSL_enums_size) {
		...
		snprintf(buf, 200,
				"SSL.VersionRangeGetDefaultNative(): for variant=%d failed: "
						"out of range for array JSSL_enums size: %d",
				variant_ssl,
				JSSL_enums_size);
		JSSL_throwSSLSocketException(env, buf);
		return NULL;
...
Java_org_mozilla_jss_nss_SSL_SecurityStatus(JNIEnv *env, jclass clazz,
	...
	if (SSL_SecurityStatus(real_fd, &on, &cipher, &keySize, &secretKeySize,
...
Java_org_mozilla_jss_nss_SSL_GetChannelInfo(JNIEnv *env, jclass clazz,
	...
	return JSS_NewSSLChannelInfo(env, pV, cS, aKB, kKB, cT, lAT, eT, sID, cM,
			eMSU, eDA, kT, kG, sC, mA, aT, sS, haveNSS334, oKG, r, haveNSS345,
			pDC);
}
...
Java_org_mozilla_jss_nss_SSL_GetPreliminaryChannelInfo(JNIEnv *env,
	...
	if (SSL_GetPreliminaryChannelInfo(real_fd, &info, sizeof(info))
			!= SECSuccess) {
		return NULL;
	}
	...
	return JSS_NewSSLPreliminaryChannelInfo(env, vS, pV, cS, cSED, mEDS,
			haveNSS343, zRCS, haveNSS348, pDC, aKB, sS);
}
...
JNIEXPORT int JNICALL
Java_org_mozilla_jss_nss_SSL_ReHandshake(JNIEnv *env, jclass clazz, jobject fd,
		jboolean flushCache) {
	PRFileDesc *real_fd = NULL;
...
JNIEXPORT int JNICALL
Java_org_mozilla_jss_nss_SSL_ConfigServerSessionIDCache(JNIEnv *env,
		jclass clazz, jint maxCacheEntries, jlong timeout, jlong ssl3_timeout,
		jstring directory) {
	const char *dir_path;
	SECStatus ret = SECFailure;
	...
	ret = SSL_ConfigServerSessionIDCache(maxCacheEntries, timeout, ssl3_timeout,
			dir_path);
...
JNIEXPORT jint JNICALL
Java_org_mozilla_jss_nss_SSL_KeyUpdate(JNIEnv *env, jclass clazz, jobject fd,
		jboolean requestUpdate) {
	PRFileDesc *real_fd = NULL;
	...
	return SSL_KeyUpdate(real_fd,
			requestUpdate == JNI_TRUE ? PR_TRUE : PR_FALSE);
}
...
Java_org_mozilla_jss_nss_SSL_EnableAlertLoggingNative(JNIEnv *env, jclass clazz,
	...
	ret = SSL_AlertReceivedCallback(real_fd, JSSL_SSLFDAlertReceivedCallback,
			fd_ref);
	if (ret != SECSuccess) {
		return ret;
	}
...
JNIEXPORT jint JNICALL
Java_org_mozilla_jss_nss_SSL_ConfigJSSDefaultCertAuthCallback(JNIEnv *env,
		jclass clazz, jobject fd) {
	PRFileDesc *real_fd = NULL;
...
JNIEXPORT jint JNICALL
Java_org_mozilla_jss_nss_SSL_ConfigAsyncTrustManagerCertAuthCallback(
	...
	return SSL_AuthCertificateHook(real_fd, JSSL_SSLFDAsyncCertAuthCallback,
			fd_ref);
}

JNIEXPORT jint JNICALL
Java_org_mozilla_jss_nss_SSL_ConfigSyncTrustManagerCertAuthCallback(JNIEnv *env,
		jclass clazz, jobject fd) {
	PRFileDesc *real_fd = NULL;
	jobject fd_ref = NULL;
	...
	return SSL_AuthCertificateHook(real_fd, JSSL_SSLFDSyncCertAuthCallback,
			fd_ref);
}

JNIEXPORT jint JNICALL
Java_org_mozilla_jss_nss_SSL_ConfigAsyncBadCertCallback(JNIEnv *env,
		jclass clazz, jobject fd) {
	PRFileDesc *real_fd = NULL;
	jobject fd_ref = NULL;
...
JNIEXPORT jint JNICALL
Java_org_mozilla_jss_nss_SSL_ConfigSyncBadCertCallback(JNIEnv *env,
		jclass clazz, jobject fd) {
	PRFileDesc *real_fd = NULL;
	jobject fd_ref = NULL;
...
JNIEXPORT jint JNICALL
Java_org_mozilla_jss_nss_SSL_getSSLEnablePostHandshakeAuth(JNIEnv *env,
		jclass clazz) {
	return SSL_ENABLE_POST_HANDSHAKE_AUTH;
}

JNIEXPORT jint JNICALL
Java_org_mozilla_jss_nss_SSL_getSSLEnableRenegotiation(JNIEnv *env,
		jclass clazz) {
	return SSL_ENABLE_RENEGOTIATION;
}

JNIEXPORT jint JNICALL
Java_org_mozilla_jss_nss_SSL_getSSLRequireSafeNegotiation(JNIEnv *env,
		jclass clazz) {
...
JNIEXPORT jint JNICALL
Java_org_mozilla_jss_nss_SSL_getSSLRenegotiateUnrestricted(JNIEnv *env,
		jclass clazz) {
	return SSL_RENEGOTIATE_UNRESTRICTED;
}

JNIEXPORT jint JNICALL
Java_org_mozilla_jss_nss_SSL_getSSLRenegotiateRequiresXtn(JNIEnv *env,
		jclass clazz) {
	return SSL_RENEGOTIATE_REQUIRES_XTN;
}

JNIEXPORT jint JNICALL
Java_org_mozilla_jss_nss_SSL_getSSLRenegotiateTransitional(JNIEnv *env,
		jclass clazz) {
...
JNIEXPORT jint JNICALL
Java_org_mozilla_jss_nss_SSL_getSSLRequireFirstHandshake(JNIEnv *env,
		jclass clazz) {
