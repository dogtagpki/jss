...
static jobject JSS_PR_wrapFDProxy(JNIEnv *env, PRFileDesc **fd,
		const char *className, const char *conSig) {
...
jobject JSS_PR_wrapPRFDProxy(JNIEnv *env, PRFileDesc **fd) {
	return JSS_PR_wrapFDProxy(env, fd, PRFD_PROXY_CLASS_NAME,
			PRFD_PROXY_CONSTRUCTOR_SIG);
}

jobject JSS_PR_wrapSSLFDProxy(JNIEnv *env, PRFileDesc **fd) {
	return JSS_PR_wrapFDProxy(env, fd, SSLFD_PROXY_CLASS_NAME,
