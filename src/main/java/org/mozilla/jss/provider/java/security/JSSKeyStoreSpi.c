...
getTokenSlotPtr(JNIEnv *env, jobject keyStoreObj, PK11SlotInfo **ptr) {
	return JSS_getPtrFromProxyOwner(env, keyStoreObj, "proxy",
			"Lorg/mozilla/jss/pkcs11/TokenProxy;", (void**) ptr);
}

typedef enum {
	PRIVKEY = 0x01, PUBKEY = 0x02, SYMKEY = 0x04, CERT = 0x08
} TokenObjectType;
...
(*TokenObjectTraversalCallback)(JNIEnv *env, PK11SlotInfo *slot,
		TokenObjectType type, void *obj, void *data);

/*
 * objectType is bitwise-OR of all the object types you want to traverse.
 */
static PRStatus traverseTokenObjects(JNIEnv *env, PK11SlotInfo *slot,
		TokenObjectTraversalCallback cb, int objectTypes, void *data) {
	...
	if (objectTypes & PRIVKEY) {
		...
		if (privkList != NULL) {
			...
			for (node = PRIVKEY_LIST_HEAD(privkList);
					!PRIVKEY_LIST_END(node, privkList); node =
	...
	stop_early: status = PR_SUCCESS;
	finish: if (privkList != NULL) {
...
static void freeObjectNick(char *nick, TokenObjectType type) {
...
static JSSTraversalStatus engineAliasesTraversalCallback(JNIEnv *env,
		PK11SlotInfo *slot, TokenObjectType type, void *obj, void *data) {
	JSSTraversalStatus travStat = INIT_TRAVSTAT;
	...
	if (nickname != NULL) {
		...
		(*env)->CallBooleanMethod(env, cbinfo->setObj, cbinfo->setAdd,
				nickString);
...
Java_org_mozilla_jss_provider_java_security_JSSKeyStoreSpi_getRawAliases(
	 ...
	 */
	if (traverseTokenObjects(env, slot, engineAliasesTraversalCallback,
	ALL_OBJECT_TYPES, &cbinfo) != PR_SUCCESS) {
		goto finish;
	}

	finish: return setObj;
}
...
static JSSTraversalStatus engineDeleteEntryTraversalCallback(JNIEnv *env,
		PK11SlotInfo *slot, TokenObjectType type, void *obj, void *data) {
...
Java_org_mozilla_jss_provider_java_security_JSSKeyStoreSpi_engineDeleteEntry(
	 ...
	 * traverse all slot objects
	 */
	traverseTokenObjects(env, slot, engineDeleteEntryTraversalCallback,
	ALL_OBJECT_TYPES, (void*) &cbinfo);

	finish: JSS_DerefJString(env, aliasString, cbinfo.targetNickname);
}
...
static JSSTraversalStatus engineGetCertificateTraversalCallback(JNIEnv *env,
		PK11SlotInfo *slot, TokenObjectType type, void *obj, void *data) {
	JSSTraversalStatus travStat = INIT_TRAVSTAT;
...
lookupCertByNickname(JNIEnv *env, jobject this, jstring alias) {
	...
	status = traverseTokenObjects(env, slot,
			engineGetCertificateTraversalCallback, CERT, (void*) &cbinfo);

	finish: JSS_DerefJString(env, alias, cbinfo.targetNickname);
...
static JSSTraversalStatus findKeyCallback(JNIEnv *env, PK11SlotInfo *slot,
		TokenObjectType type, void *obj, void *data) {
	JSSTraversalStatus status = INIT_TRAVSTAT;
...
static JSSTraversalStatus getKeyByCertNickCallback(JNIEnv *env,
		PK11SlotInfo *slot, TokenObjectType type, void *obj, void *data) {
	JSSTraversalStatus travStat = INIT_TRAVSTAT;
	CERTCertificate *cert = (CERTCertificate*) obj;
...
Java_org_mozilla_jss_provider_java_security_JSSKeyStoreSpi_engineGetKeyNative(
	...
	if (traverseTokenObjects(env, slot, findKeyCallback, PRIVKEY | SYMKEY,
			&keyCbinfo) != PR_SUCCESS) {
		goto finish;
	}
	...
	if (keyObj == NULL) {
		...
		if (traverseTokenObjects(env, slot, getKeyByCertNickCallback, CERT,
				(void*) &certCbinfo) != PR_SUCCESS) {
			goto finish;
		}
	...
	finish: JSS_DerefJString(env, alias, keyCbinfo.label);
...
Java_org_mozilla_jss_provider_java_security_JSSKeyStoreSpi_engineIsCertificateEntry(
	...
	if (traverseTokenObjects(env, slot, engineGetCertificateTraversalCallback,
			CERT, (void*) &cbinfo) != PR_SUCCESS) {
		goto finish;
	}
	...
	if (cbinfo.cert != NULL) {
		...
		if ((allTrust
				& (CERTDB_TRUSTED | CERTDB_TRUSTED_CA | CERTDB_NS_TRUSTED_CA
...
Java_org_mozilla_jss_provider_java_security_JSSKeyStoreSpi_engineSetKeyEntryNative(
	...
	finish: JSS_DerefJString(env, alias, nickname);
