...
Java_org_mozilla_jss_pkcs11_PK11Cipher_initContext(JNIEnv *env, jclass clazz,
		jboolean encrypt, jobject keyObj, jobject algObj, jbyteArray ivBA,
		jboolean padded) {
	return Java_org_mozilla_jss_pkcs11_PK11Cipher_initContextWithKeyBits(env,
			clazz, encrypt, keyObj, algObj, ivBA, 0, padded);
}

JNIEXPORT jobject JNICALL
Java_org_mozilla_jss_pkcs11_PK11Cipher_initContextWithKeyBits(JNIEnv *env,
		jclass clazz, jboolean encrypt, jobject keyObj, jobject algObj,
		jbyteArray ivBA, jint keyBits, jboolean padded) {
	...
	if (context == NULL) {
		JSS_throwMsg(env, TOKEN_EXCEPTION, "Failed to generate crypto context");
		goto finish;
	}
...
Java_org_mozilla_jss_pkcs11_PK11Cipher_updateContext(JNIEnv *env, jclass clazz,
		jobject contextObj, jbyteArray inputBA, jint blockSize) {
	PK11Context *context = NULL;
	jbyte *inbuf = NULL;
	unsigned int inlen;
	...
	PR_ASSERT(
			env != NULL && clazz != NULL && contextObj != NULL
					&& inputBA != NULL);
	...
	if (PK11_CipherOp(context, outbuf, (int*) &outlen, outlen,
		...
		JSS_throwMsgPrErrArg(env, TOKEN_EXCEPTION,
				"Cipher context update failed", PR_GetError());
		goto finish;
	}
 ...
 */
JNIEXPORT jbyteArray JNICALL
Java_org_mozilla_jss_pkcs11_PK11Cipher_finalizeContext(JNIEnv *env,
		jclass clazz, jobject contextObj, jint blockSize, jboolean padded) {
	PK11Context *context = NULL;
	unsigned char *outBuf = NULL;
	...
	if ((status != SECSuccess)) {
		JSS_throwMsgPrErrArg(env, TOKEN_EXCEPTION,
				"Cipher context finalization failed", PR_GetError());
		goto finish;
	}
 ...
 */
PRStatus JSS_PK11_getCipherContext(JNIEnv *env, jobject proxy,
		PK11Context **pContext) {
 ...
 */
jobject JSS_PK11_wrapCipherContextProxy(JNIEnv *env, PK11Context **context) {
