...
JNIEXPORT jstring JNICALL Java_org_mozilla_jss_pkcs11_PK11Module_getName(
	...
	finish: PR_ASSERT(nameString || (*env)->ExceptionOccurred(env));
	return nameString;
}
...
JNIEXPORT jstring JNICALL Java_org_mozilla_jss_pkcs11_PK11Module_getLibraryName(
	...
	finish: PR_ASSERT(libName || (*env)->ExceptionOccurred(env));
	return libName;
}
 ...
 */
jobject JSS_PK11_wrapPK11Module(JNIEnv *env, SECMODModule **module) {
	jclass moduleClass;
	...
	constructor = (*env)->GetMethodID(env, moduleClass,
	PLAIN_CONSTRUCTOR,
 ...
 *      peration succeeded.
 */
PRStatus JSS_PK11_getModulePtr(JNIEnv *env, jobject module, SECMODModule **ptr) {
	PR_ASSERT(env != NULL && module != NULL && ptr != NULL);

	return JSS_getPtrFromProxyOwner(env, module,
	PK11MODULE_PROXY_FIELD,
	PK11MODULE_PROXY_SIG, (void**) ptr);
}
...
Java_org_mozilla_jss_pkcs11_ModuleProxy_releaseNativeResources(JNIEnv *env,
		jobject this) {
