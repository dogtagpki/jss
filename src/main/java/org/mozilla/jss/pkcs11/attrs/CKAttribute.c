...
PRStatus JSS_PK11_WrapAttribute(JNIEnv *env, jobject this, void *ptr,
		size_t ptr_length) {
	jclass this_class;
	jfieldID field_id;
	jobject ptr_object;
	...
	if (JSS_PR_StoreNativeEnclosure(env, this, ptr_object, sizeof(CK_ATTRIBUTE))
			!= PR_SUCCESS) {
		goto failure;
	}
	...
	failure: memset(attr, 0, sizeof(CK_ATTRIBUTE));
	free(attr);
	return PR_FAILURE;
}

PRStatus JSS_PK11_UnwrapAttribute(JNIEnv *env, jobject this,
	...
	if (JSS_PR_getStaticVoidRef(env, ptr_obj, (void**) attr) != PR_SUCCESS
			|| *attr == NULL) {
		goto failure;
	}
...
Java_org_mozilla_jss_pkcs11_attrs_CKAClass_acquireNativeResources(JNIEnv *env,
		jobject this) {
	jclass this_class;
	jfieldID field_id;
	...
	*ptr = (CK_ULONG)((*env)->GetLongField(env, this, field_id));
	...
	if (JSS_PK11_WrapAttribute(env, this, (void*) ptr, sizeof(*ptr))
			== PR_FAILURE) {
		goto failure;
	}
	...
	failure: memset(ptr, 0, sizeof(*ptr));
	free(ptr);
}

JNIEXPORT void JNICALL
Java_org_mozilla_jss_pkcs11_attrs_CKAClass_releaseNativeResources(JNIEnv *env,
		jobject this) {
	CK_ATTRIBUTE_PTR attr = NULL;

	if (JSS_PK11_UnwrapAttribute(env, this, &attr) != PR_SUCCESS
			|| attr == NULL) {
		return;
	}
...
JNIEXPORT void JNICALL
Java_org_mozilla_jss_pkcs11_attrs_CKAKeyType_acquireNativeResources(JNIEnv *env,
		jobject this) {
	jclass this_class;
	jfieldID field_id;
	CK_ULONG *ptr = calloc(1, sizeof(CK_ULONG));
...
JNIEXPORT void JNICALL
Java_org_mozilla_jss_pkcs11_attrs_CKAKeyType_releaseNativeResources(JNIEnv *env,
		jobject this) {
	CK_ATTRIBUTE_PTR attr = NULL;
...
JNIEXPORT void JNICALL
Java_org_mozilla_jss_pkcs11_attrs_CKAUsage_acquireNativeResources(JNIEnv *env,
		jobject this) {
	JSS_PK11_WrapAttribute(env, this, (void*) &JSS_CK_TRUE,
			sizeof(JSS_CK_TRUE));
}

JNIEXPORT void JNICALL
Java_org_mozilla_jss_pkcs11_attrs_CKAUsage_releaseNativeResources(JNIEnv *env,
		jobject this) {
	CK_ATTRIBUTE_PTR attr = NULL;
