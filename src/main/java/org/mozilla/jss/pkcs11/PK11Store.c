...
Java_org_mozilla_jss_pkcs11_PK11Store_putSymKeysInVector(JNIEnv *env,
		jobject this, jobject keyVector) {
	PK11SlotInfo *slot;
	jobject object = NULL;
	jclass vectorClass;
 ...
 */
JNIEXPORT void JNICALL
Java_org_mozilla_jss_pkcs11_PK11Store_loadPrivateKeys(JNIEnv *env, jobject this,
		jobject collection) {
	PK11SlotInfo *slot;
	...
	collectionAdd = (*env)->GetMethodID(env, collectionClass,
	COLLECTION_ADD_NAME,
	COLLECTION_ADD_SIG);
	...
	for (node = PRIVKEY_LIST_HEAD(list); !PRIVKEY_LIST_END(node, list); node =
 ...
 */
JNIEXPORT void JNICALL
Java_org_mozilla_jss_pkcs11_PK11Store_loadPublicKeys(JNIEnv *env, jobject this,
		jobject collection) {
	PK11SlotInfo *slot;
	...
	for (node = PUBKEY_LIST_HEAD(list); !PUBKEY_LIST_END(node, list); node =
 ...
 */
JNIEXPORT void JNICALL
Java_org_mozilla_jss_pkcs11_PK11Store_putCertsInVector(JNIEnv *env,
		jobject this, jobject certVector) {
	PK11SlotInfo *slot;
	...
	for (node = CERT_LIST_HEAD(certList); !CERT_LIST_END(node, certList); node =
		...
		object = JSS_PK11_wrapCertAndSlotAndNickname(env, &certCopy, &slotCopy,
				node->appData);
		if (object == NULL) {
			PR_ASSERT((*env)->ExceptionOccurred(env));
			goto finish;
 ...
 */
PRStatus JSS_PK11_getStoreSlotPtr(JNIEnv *env, jobject store,
		PK11SlotInfo **slot) {
 ...
 */
JNIEXPORT void JNICALL
Java_org_mozilla_jss_pkcs11_PK11Store_deletePrivateKey(JNIEnv *env,
		jobject this, jobject privateKeyObj) {
	PK11SlotInfo *slot;
	...
	if (PK11_DestroyTokenObject(privateKey->pkcs11Slot, privateKey->pkcs11ID)
			!= SECSuccess) {
 ...
 */
JNIEXPORT void JNICALL
Java_org_mozilla_jss_pkcs11_PK11Store_deletePublicKey(JNIEnv *env, jobject this,
		jobject publicKeyObj) {
	PK11SlotInfo *slot;
	...
	if (PK11_DestroyTokenObject(publicKey->pkcs11Slot, publicKey->pkcs11ID)
			!= SECSuccess) {
 ...
 */
JNIEXPORT void JNICALL
Java_org_mozilla_jss_pkcs11_PK11Store_deleteCert(JNIEnv *env, jobject this,
		jobject certObject) {
 ...
 */
JNIEXPORT void JNICALL
Java_org_mozilla_jss_pkcs11_PK11Store_deleteCertOnly(JNIEnv *env, jobject this,
		jobject certObject) {
	CERTCertificate *cert;
	SECStatus VARIABLE_MAY_NOT_BE_USED status;
...
Java_org_mozilla_jss_pkcs11_PK11Store_importPrivateKey(JNIEnv *env,
		jobject this, jbyteArray keyArray, jobject keyTypeObj,
		jboolean temporary) {
	SECItem derPK;
	PK11SlotInfo *slot;
	jthrowable excep;
	 ...
	 */
	if (!JSS_RefByteArray(env, keyArray, (jbyte**) &derPK.data,
			(jsize*) &derPK.len)) {
		if (derPK.len == 0) {
	...
	status = PK11_ImportDERPrivateKeyInfoAndReturnKey(slot, &derPK, &nickname,
			NULL /*public value*/, !temporary /*isPerm*/, PR_TRUE /*isPrivate*/,
			0 /*keyUsage*/, &privk, NULL /*wincx*/);
...
Java_org_mozilla_jss_pkcs11_PK11Store_getEncryptedPrivateKeyInfo(JNIEnv *env,
		jobject this, jobject conv, jobject pwObj, jobject algObj,
		jint iterations, jobject key) {
	...
	// export the epki
	epki = PK11_ExportEncryptedPrivKeyInfo(slot, algTag, pwItem, privk,
			iterations, NULL /*wincx*/);
	if (epki == NULL) {
		JSS_throwMsgPrErr(env, TOKEN_EXCEPTION,
				"Failed to export EncryptedPrivateKeyInfo");
		goto finish;
	}
...
JNIEXPORT void JNICALL
Java_org_mozilla_jss_pkcs11_PK11Store_importEncryptedPrivateKeyInfo(JNIEnv *env,
		jobject this, jobject conv, jobject pwObj, jstring nickname,
		jobject pubKeyObj, jbyteArray epkiBytes) {
	// initialisations so we can goto finish
	...
	if (SEC_ASN1DecodeItem(NULL, epki,
			SEC_ASN1_GET(SECKEY_EncryptedPrivateKeyInfoTemplate), epkiItem)
			!= SECSuccess) {
	...
	jmethodID getEncoded = (*env)->GetMethodID(env, pubKeyClass, "getEncoded",
			"()[B");
	if (getEncoded == NULL) {
		ASSERT_OUTOFMEM(env);
		goto finish;
	}
	jbyteArray spkiBytes = (*env)->CallObjectMethod(env, pubKeyObj, getEncoded);
	spkiItem = JSS_ByteArrayToSECItem(env, spkiBytes);
	spki = PR_Calloc(1, sizeof(CERTSubjectPublicKeyInfo));
	if (SEC_ASN1DecodeItem(NULL, spki,
			SEC_ASN1_GET(CERT_SubjectPublicKeyInfoTemplate), spkiItem)
			!= SECSuccess) {
		JSS_throwMsg(env, INVALID_DER_EXCEPTION,
	...
	// perform import
	SECStatus result = PK11_ImportEncryptedPrivateKeyInfo(slot, epki, pwItem,
			&nickItem, pubValue, PR_TRUE /* isperm */, PR_TRUE /* isprivate */,
...
SECItem* preparePassword(JNIEnv *env, jobject conv, jobject pwObj) {
	...
	if (conv == NULL) {
		jmethodID getByteCopy = (*env)->GetMethodID(env, passwordClass,
				PW_GET_BYTE_COPY_NAME, PW_GET_BYTE_COPY_SIG);
		if (getByteCopy == NULL) {
			ASSERT_OUTOFMEM(env);
			return NULL;
		}
		...
		jmethodID getChars = (*env)->GetMethodID(env, passwordClass, "getChars",
				"()[C");
		if (getChars == NULL) {
			ASSERT_OUTOFMEM(env);
			return NULL;
		...
		jmethodID convert = (*env)->GetMethodID(env, convClass, "convert",
				"([C)[B");
		if (convert == NULL) {
			ASSERT_OUTOFMEM(env);
			return NULL;
