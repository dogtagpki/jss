...
JNIEXPORT jobject JNICALL Java_org_mozilla_jss_pkcs11_PK11SymmetricKeyDeriver_nativeDeriveSymKey(
		JNIEnv *env, jobject this, jobject tokenObj, jobject baseKeyObj,
		jobject secondaryKeyObj, jlong deriveMechanism, jbyteArray param,
		jbyteArray iv, jlong targetMechanism, jlong operation, jlong keySize) {
	jobject keyObj = NULL;
	...
	case CKM_DES3_CBC_ENCRYPT_DATA:
		...
		if (ivValue == NULL) {
			PR_fprintf(PR_STDOUT,
					"Need iv param for CKM_DES_CBC_ENCRYPT_DATA or CKM_DES3_CBC_ENCRYPT_DATA. \n");
			goto finish;
		}

		if (ivLength != 8) {
			PR_fprintf(PR_STDOUT,
					"Need iv param for CKM_DES_CBC_ENCRYPT_DATA  structure to be 8 bytes!. \n");
			goto finish;
		}
	...
	case CKM_SEED_CBC_ENCRYPT_DATA:
		...
		if (ivValue == NULL) {
			PR_fprintf(PR_STDOUT,
					"Need iv param for CBC encrypt derive for AES, or CAMELLIA or SEED. \n");
			goto finish;
		}

		if (ivLength != 16) {
			PR_fprintf(PR_STDOUT,
					"Need iv param for CK_AES_CBC_ENCRYPT_DATA_PARAMS structure to be 16 bytes!. \n");
			goto finish;
		}
	...
	if (JSS_PK11_getSymKeyPtr(env, baseKeyObj, &baseKey) != PR_SUCCESS) {
		PR_fprintf(PR_STDOUT,
				"PK11SymmetricKeyDeriver.nativeDeriveSymKey: Unable to extract symmetric base key!");
		goto finish;
	}
	...
	if (bestSlot == NULL) {
		PR_fprintf(PR_STDOUT,
				"PK11SymmetricKeyDeriver.nativeDeriveSymKey: Can't find suitable slot for sym key derivation! \n");
		goto finish;
	}
	...
	if (PK11_DoesMechanism(slot, deriveMechanism)) {
		if (keyOnRequestedSlot) {
			...
			bestBaseKey = PK11_MoveSymKey(slot, CKA_ENCRYPT, 0, PR_FALSE,
					baseKey);
			if (bestBaseKey == NULL) {
				PR_fprintf(PR_STDOUT,
						"PK11SymmetricKeyDeriver.nativeDeriveSymKey:  Can't move Base Key to requested slot!\n");
				goto finish;
			}
		...
		bestBaseKey = PK11_MoveSymKey(bestSlot, CKA_ENCRYPT, 0, PR_FALSE,
				baseKey);
		if (bestBaseKey == NULL) {
			PR_fprintf(PR_STDOUT,
					"PK11SymmetricKeyDeriver.nativeDeriveSymKey:  Can't move Base Key to best slot!\n");
			goto finish;
		}
		finalBaseKey = bestBaseKey;
	...
	if (secondaryKeyObj != NULL) {
		if (JSS_PK11_getSymKeyPtr(env, secondaryKeyObj, &secondaryKey)
				!= PR_SUCCESS) {
			PR_fprintf(PR_STDOUT,
					"PK11SymmetricKeyDeriver.nativeDeriveSymKey:  Can't find secondary sym key!\n");
			goto finish;
		}
		...
		if (finalBaseKeySlot != slotForSecondaryKey) {
			...
			finalSecondaryKey = PK11_MoveSymKey(finalBaseKeySlot, CKA_ENCRYPT,
					0, PR_FALSE, secondaryKey);

			if (finalSecondaryKey == NULL) {
				PR_fprintf(PR_STDOUT,
						"PK11SymmetricKeyDeriver.nativeDeriveSymKey, Problem moving secondary key to proper slot.\n");
				goto finish;
			}
		} else {
		...
		if (paramValue == NULL) {
			...
			if (keyhandle == 0) {
				PR_fprintf(PR_STDOUT,
						"PK11SymmetricKeyDeriver.nativeDeriveSymKey, can't get handle for secondary sym key.\n");
				goto finish;
			}
			...
			PR_fprintf(PR_STDOUT,
					"PK11SymmetricKeyDeriver.nativeDeriveSymKey: incorrect input parameter provided!\n");
			goto finish;
		}
	}

	derivedKey = PK11_Derive(finalBaseKey, deriveMechanism, &paramsItem,
			targetMechanism, operation, keySize);
	if (derivedKey == NULL) {
		PR_fprintf(PR_STDOUT, "ERROR: Can't derive symmetric key, error: %d \n",
				PR_GetError());
		goto finish;
	}
	...
	if ((finalSlot = PK11_GetSlotFromKey(derivedKey)) != slot) {
		...
		if (newKey == NULL) {
			PR_fprintf(PR_STDOUT,
					"PK11SymmetricKeyDeriver.nativeDeriveSymKey: error moving key to original slot, return anyway. \n");
			newKey = derivedKey;
			derivedKey = NULL;
		}
	...
	if (keyObj == NULL) {
		JSS_throwMsgPrErr(env, TOKEN_EXCEPTION,
				"Unable to derive symmetric key! "
						"failure!");
	}
