 ...
 */
static jobject
keysToKeyPair(JNIEnv *env, SECKEYPrivateKey **pPrivk, SECKEYPublicKey **pPubk) {
	jobject privateKey;
	jobject publicKey;
	...
	keyPairConstructor = (*env)->GetMethodID(env, keyPairClass,
	...
	keyPair = (*env)->NewObject(env, keyPairClass, keyPairConstructor,
	...
	finish: return keyPair;
}

int PK11_NumberObjectsFor(PK11SlotInfo*, CK_ATTRIBUTE*, int);

SECStatus JSS_PK11_generateKeyPairWithOpFlags(JNIEnv *env,
		CK_MECHANISM_TYPE mechanism, PK11SlotInfo *slot, SECKEYPublicKey **pubk,
		SECKEYPrivateKey **privk, void *params, PRBool temporary,
		jint sensitive, jint extractable, jint op_flags, jint op_flags_mask) {
	...
	*pubk = NULL;
	 ...
	 *************************************************/
	if (PK11_Authenticate(slot, PR_TRUE /*loadcerts*/, NULL) != SECSuccess) {
	...
	*privk = PK11_GenerateKeyPairWithOpFlags(slot, mechanism, params, pubk,
			attrFlags, (CK_FLAGS) op_flags, (CK_FLAGS) op_flags_mask
			/* the ones we don't want*/, NULL /* default PW callback */);
	...
	if (*privk == NULL) {
		...
		msgBuf = PR_smprintf(
				"Keypair Generation failed on token with error: %d : %s",
				PR_GetError(), errLength > 0 ? errBuf : "");
		if (errLength > 0) {
			PR_Free(errBuf);
		}
 ...
 */
SECStatus JSS_PK11_generateKeyPair(JNIEnv *env, CK_MECHANISM_TYPE mechanism,
...
static jobject PK11KeyPairGeneratorWithOpFlags(JNIEnv *env, jobject this,
		jobject token, CK_MECHANISM_TYPE mechanism, void *params,
		jboolean temporary, jint sensitive, jint extractable, jint op_flags,
		jint op_flags_mask) {
	PK11SlotInfo *slot;
	...
	rv = JSS_PK11_generateKeyPairWithOpFlags(env, mechanism, slot, &pubk,
			&privk, params, temporary, sensitive, extractable, op_flags,
			op_flags_mask);
	if (rv != SECSuccess) {
		goto finish;
	}
...
static jobject PK11KeyPairGenerator(JNIEnv *env, jobject this, jobject token,
		CK_MECHANISM_TYPE mechanism, void *params, jboolean temporary,
		jint sensitive, jint extractable) {
	return PK11KeyPairGeneratorWithOpFlags(env, this, token, mechanism, params,
			temporary, sensitive, extractable, 0, 0);
}
...
Java_org_mozilla_jss_pkcs11_PK11KeyPairGenerator_generateRSAKeyPair(JNIEnv *env,
		jobject this, jobject token, jint keySize, jlong publicExponent,
...
Java_org_mozilla_jss_pkcs11_PK11KeyPairGenerator_generateRSAKeyPairWithOpFlags(
		JNIEnv *env, jobject this, jobject token, jint keySize,
		jlong publicExponent, jboolean temporary, jint sensitive,
		jint extractable, jint op_flags, jint op_flags_mask) {
	PK11RSAGenParams params;
	...
	return PK11KeyPairGeneratorWithOpFlags(env, this, token,
			CKM_RSA_PKCS_KEY_PAIR_GEN, &params, temporary, sensitive,
			extractable, op_flags, op_flags_mask);
}
 ...
 */
JNIEXPORT jobject JNICALL
Java_org_mozilla_jss_pkcs11_PK11KeyPairGenerator_generateDSAKeyPair(JNIEnv *env,
		jobject this, jobject token, jbyteArray P, jbyteArray Q, jbyteArray G,
		jboolean temporary, jint sensitive, jint extractable) {
	SECItem p, q, g;
	...
	PR_ASSERT(
			env != NULL && this != NULL && token != NULL && P != NULL
					&& Q != NULL && G != NULL);
	...
	finish: SECITEM_FreeItem(&p, PR_FALSE);
...
Java_org_mozilla_jss_pkcs11_PK11KeyPairGenerator_generateDSAKeyPairWithOpFlags(
	...
	keyPair = PK11KeyPairGeneratorWithOpFlags(env, this, token,
			CKM_DSA_KEY_PAIR_GEN, params, temporary, sensitive, extractable,
 ...
 */
JNIEXPORT jobject JNICALL
Java_org_mozilla_jss_pkcs11_PK11KeyPairGenerator_generateECKeyPair(JNIEnv *env,
		jobject this, jobject token, jbyteArray Curve, jboolean temporary,
		jint sensitive, jint extractable) {
	...
	finish: SECITEM_FreeItem(&curve, PR_FALSE);
	return keyPair;
}
...
Java_org_mozilla_jss_pkcs11_PK11KeyPairGenerator_generateECKeyPairWithOpFlags(
	...
	keyPair = PK11KeyPairGeneratorWithOpFlags(env, this, token,
			CKM_EC_KEY_PAIR_GEN, &curve, temporary, sensitive, extractable,
			op_flags, op_flags_mask);
