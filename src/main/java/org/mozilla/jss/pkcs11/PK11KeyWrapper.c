...
Java_org_mozilla_jss_pkcs11_PK11KeyWrapper_nativeWrapSymWithSym(JNIEnv *env,
		jobject this, jobject tokenObj, jobject toBeWrappedObj,
	...
	/* get toBeWrapped key */
	if (JSS_PK11_getSymKeyPtr(env, toBeWrappedObj, &toBeWrapped)
			!= PR_SUCCESS) {
		JSS_throwMsg(env, TOKEN_EXCEPTION, "Unable to extract symmetric "
				"to be wrapped key");
		return NULL;
 ...
 */
JNIEXPORT jbyteArray JNICALL
Java_org_mozilla_jss_pkcs11_PK11KeyWrapper_nativeWrapSymWithPub(JNIEnv *env,
		jobject this, jobject tokenObj, jobject toBeWrappedObj,
	...
	if (mech == CKM_RSA_PKCS_OAEP) {
		JSS_throwMsg(env, TOKEN_EXCEPTION,
				"RSA-OAEP not supported by the NSS version used to build JSS");
		goto finish;
	}
	...
	finish: SECITEM_FreeItem(&wrapped, PR_FALSE /*freeit*/);
 ...
 */
JNIEXPORT jbyteArray JNICALL
Java_org_mozilla_jss_pkcs11_PK11KeyWrapper_nativeWrapPrivWithSym(JNIEnv *env,
		jobject this, jobject tokenObj, jobject toBeWrappedObj,
		jobject wrappingKeyObj, jobject algObj, jbyteArray ivBA) {
	PK11SymKey *wrapping = NULL;
	...
	/* get toBeWrapped key */
	if (JSS_PK11_getPrivKeyPtr(env, toBeWrappedObj, &toBeWrapped)
			!= PR_SUCCESS) {
...
Java_org_mozilla_jss_pkcs11_PK11KeyWrapper_nativeUnwrapPrivWithSym(JNIEnv *env,
		jclass clazz, jobject tokenObj, jobject unwrapperObj,
	...
	if (privk == NULL) {
		char err[256] = { 0 };
		PR_snprintf(err, 256, "Key Unwrap failed on token; keyType=%d",
				keyType);
 ...
 */
JNIEXPORT jobject JNICALL
Java_org_mozilla_jss_pkcs11_PK11KeyWrapper_nativeUnwrapSymWithSym(JNIEnv *env,
		jclass clazz, jobject tokenObj, jobject unwrapperObj,
		jbyteArray wrappedBA, jobject wrapAlgObj, jobject typeAlgObj,
	...
	if (isPermanent == PR_FALSE) {
		...
		symKey = PK11_UnwrapSymKeyWithFlagsPerm(wrappingKey, wrappingMech,
				param, wrappedKey, keyTypeMech, operation, keyLen, flags,
				isPermanent);
	}
 ...
 */
JNIEXPORT jobject JNICALL
Java_org_mozilla_jss_pkcs11_PK11KeyWrapper_nativeUnwrapSymWithPriv(JNIEnv *env,
		jclass clazz, jobject tokenObj, jobject unwrapperObj,
		jbyteArray wrappedBA, jobject wrapAlgObj, jobject typeAlgObj,
...
PK11SymKey* JSS_PK11_ImportSymKeyWithFlagsFIPS(PK11SlotInfo *slot,
...
#define FIPS_ENCRYPT_UNWRAP_ALGO CKM_AES_KEY_WRAP_PAD
	...
	ret = PK11_Encrypt(wrapper, FIPS_ENCRYPT_UNWRAP_ALGO, param, wrapped_key,
			&wrapped_len, wrapped_max, key->data, key->len);
	if (ret != SECSuccess) {
		goto done;
	}
	...
	result = PK11_UnwrapSymKeyWithFlagsPerm(wrapper, FIPS_ENCRYPT_UNWRAP_ALGO,
			param, &wrapped_item, type, operation, key->len, flags, isPerm);

	done: free(wrapped_key);
 ...
 */
JNIEXPORT jobject JNICALL
Java_org_mozilla_jss_pkcs11_PK11KeyWrapper_nativeUnwrapSymPlaintext(JNIEnv *env,
		jclass clazz, jobject tokenObj, jbyteArray wrappedBA,
	...
	/* pull in the key */
	if (PK11_IsFIPS()) {
		symKey = JSS_PK11_ImportSymKeyWithFlagsFIPS(slot, keyTypeMech,
				operation, wrappedKey, flags, isPerm, NULL);
	} else {
		symKey = PK11_ImportSymKeyWithFlags(slot, keyTypeMech,
				PK11_OriginUnwrap, operation, wrappedKey, flags, isPerm, NULL);
	}
	if (symKey == NULL) {
		JSS_throwMsgPrErr(env, TOKEN_EXCEPTION, "Failed to unwrap key");
		goto finish;
...
CK_MECHANISM_TYPE getSupportedWrappingMechanism(JNIEnv *env, jobject algObj,
	 ...
	 */
	if (mech == CKM_AES_KEY_WRAP || mech == CKM_NSS_AES_KEY_WRAP) {
