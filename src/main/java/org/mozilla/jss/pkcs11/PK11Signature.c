...
Java_org_mozilla_jss_pkcs11_PK11Signature_initSigContext(JNIEnv *env,
		jobject this) {
	...
	if (ctxt == NULL) {
		JSS_throwMsgPrErr(env, TOKEN_EXCEPTION,
				"Unable to create signing context");
		goto finish;
	}

	if (SGN_Begin(ctxt) != SECSuccess) {
		JSS_throwMsgPrErr(env, TOKEN_EXCEPTION,
				"Unable to begin signing context");
		goto finish;
	}
	...
	finish: if (contextProxy == NULL && ctxt != NULL) {
...
JNIEXPORT void JNICALL
Java_org_mozilla_jss_pkcs11_PK11Signature_initVfyContext(JNIEnv *env,
		jobject this) {
	...
	if (signingAlg == SEC_OID_PKCS1_RSA_PSS_SIGNATURE) {
		...
		ctxt = VFY_CreateContextWithAlgorithmID(pubk, NULL, signAlg, &digestAlg,
				NULL);
	} else {
 ...
 */
JNIEXPORT void JNICALL
Java_org_mozilla_jss_pkcs11_PK11Signature_engineUpdateNative(JNIEnv *env,
		jobject this, jbyteArray bArray, jint offset, jint length) {
	SigContextType type;
	void *ctxt;
	jbyte *bytes = NULL;
	jint numBytes;
	...
	/* Update the context */
	if (type == SGN_CONTEXT) {
		if (SGN_Update((SGNContext*) ctxt, (unsigned char*) bytes + offset,
		...
		if (VFY_Update((VFYContext*) ctxt, (unsigned char*) bytes + offset,
				(unsigned) length) != SECSuccess) {
			JSS_throwMsgPrErr(env, SIGNATURE_EXCEPTION, "update failed");
			goto finish;
		}
	...
	finish: JSS_DerefByteArray(env, bArray, bytes, JNI_ABORT);
}
...
Java_org_mozilla_jss_pkcs11_PK11Signature_engineSignNative(JNIEnv *env,
		jobject this) {
	SGNContext *ctxt;
	SigContextType type;
	SECItem signature;
	...
	if (SGN_End(ctxt, &signature) != SECSuccess) {
		JSS_throwMsgPrErr(env, SIGNATURE_EXCEPTION, "Signing operation failed");
		goto finish;
	}
...
Java_org_mozilla_jss_pkcs11_PK11Signature_engineVerifyNative(JNIEnv *env,
		jobject this, jbyteArray sigArray) {
	 ...
	 */
	if (!JSS_RefByteArray(env, sigArray, (jbyte**) &sigItem.data,
			(jsize*) &sigItem.len)) {
		ASSERT_OUTOFMEM(env);
		goto finish;
	}
	...
	finish: JSS_DerefByteArray(env, sigArray, sigItem.data, JNI_ABORT);
	return verified;
}
 ...
 * 		error occurs.
 */
static SECOidTag getAlgorithm(JNIEnv *env, jobject sig) {
	jclass sigClass;
	jfieldID algField;
	jobject alg;
...
static SECOidTag getDigestAlgorithm(JNIEnv *env, jobject sig) {
	jclass sigClass;
	jfieldID algField;
	jobject alg;
...
static SECStatus getRSAPSSParamsAndSigningAlg(JNIEnv *env, jobject this,
		PRArenaPool *arena, SECAlgorithmID **alg, SECKEYPrivateKey *privk) {
	...
	sigAlgParams = SEC_CreateSignatureAlgorithmParameters(arena, NULL,
			SEC_OID_PKCS1_RSA_PSS_SIGNATURE, digestAlg, NULL, privk);
 ...
 **/
static void setSigContext(JNIEnv *env, jobject sig, jobject context) {
	jclass sigClass;
	...
	contextField = (*env)->GetFieldID(env, sigClass,
 ...
 */
static PRStatus getSigContext(JNIEnv *env, jobject sig, void **pContext,
 ...
 */
static PRStatus getPrivateKey(JNIEnv *env, jobject sig, SECKEYPrivateKey **key) {
 ...
 */
static PRStatus getPublicKey(JNIEnv *env, jobject sig, SECKEYPublicKey **key) {
	return getSomeKey(env, sig, (void**) key, PUBLICKEYTYPE);
}

static PRStatus getSomeKey(JNIEnv *env, jobject sig, void **key, short type) {
	jfieldID keyField;
	jclass sigClass;
	jobject keyProxy;
 ...
 */
PRStatus JSS_PK11_getSigContext(JNIEnv *env, jobject proxy, void **pContext,
		SigContextType *pType) {
	SigContextProxy *ctxtProxy;

	PR_ASSERT(
			env != NULL && proxy != NULL && pContext != NULL && pType != NULL);
 ...
 */
jobject JSS_PK11_wrapSigContextProxy(JNIEnv *env, void **ctxt,
		SigContextType type, PRArenaPool **arena) {
	jclass proxyClass;
 ...
 */
JNIEXPORT void JNICALL
Java_org_mozilla_jss_pkcs11_SigContextProxy_releaseNativeResources(JNIEnv *env,
		jobject this) {
	SigContextProxy *proxy = NULL;
 ...
 */
JNIEXPORT jbyteArray JNICALL
Java_org_mozilla_jss_pkcs11_PK11Signature_engineRawSignNative(JNIEnv *env,
		jclass clazz, jobject tokenObj, jobject keyObj, jbyteArray hashBA) {
	...
	PR_ASSERT(
			env != NULL && tokenObj != NULL && keyObj != NULL
					&& hashBA != NULL);
 ...
 */
JNIEXPORT jboolean JNICALL
Java_org_mozilla_jss_pkcs11_PK11Signature_engineRawVerifyNative(JNIEnv *env,
		jclass clazz, jobject tokenObj, jobject keyObj, jbyteArray hashBA,
		jbyteArray sigBA) {
	SECItem *sig = NULL;
	SECItem *hash = NULL;
	...
	PR_ASSERT(
			env != NULL && tokenObj != NULL && keyObj != NULL && hashBA != NULL
					&& sigBA != NULL);
