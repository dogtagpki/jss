 ...
 **      return -1;
 */
void JSS_throwMsgPrErrArg(JNIEnv *env, const char *throwableClassName,
 ...
 **      return -1;
 */
void JSS_throwMsg(JNIEnv *env, const char *throwableClassName,
		const char *message) {
 ...
 **      return -1;
 */
void JSS_throw(JNIEnv *env, char *throwableClassName) {
	jclass throwableClass;
	jobject throwable;
	...
	constructor = (*env)->GetMethodID(env, throwableClass,
	PLAIN_CONSTRUCTOR,
 ...
 **  }
 */
PRStatus JSS_getPtrFromProxy(JNIEnv *env, jobject nativeProxy, void **ptr) {
#ifdef DEBUG
	...
	byteArrayField = (*env)->GetFieldID(env, proxyClass,
 ...
 **  }
 */
PRStatus JSS_getPtrFromProxyOwner(JNIEnv *env, jobject proxyOwner,
		char *proxyFieldName, char *proxyFieldSig, void **ptr) {
	jclass ownerClass;
	jfieldID proxyField;
	jobject proxyObject;

	PR_ASSERT(
			env != NULL && proxyOwner != NULL && proxyFieldName != NULL
					&& ptr != NULL);
 ...
 ** was thrown.
 */
jbyteArray JSS_ptrToByteArray(JNIEnv *env, void *ptr) {
 ...
 *
 */
jbyteArray JSS_OctetStringToByteArray(JNIEnv *env, SECItem *item) {
	jbyteArray array;
	jbyte *bytes;
 ...
 *      was thrown.
 */
PRStatus JSS_ByteArrayToOctetString(JNIEnv *env, jbyteArray byteArray,
		SECItem *item) {
	jbyte *bytes = NULL;
	...
	finish: JSS_DerefByteArray(env, byteArray, bytes, JNI_ABORT);
 ...
 */
void JSS_assertOutOfMem(JNIEnv *env) {
 ...
 *      thrown.
 */
jbyteArray JSS_SECItemToByteArray(JNIEnv *env, SECItem *item) {
...
void JSS_SECStatusToExceptionMessage(JNIEnv *env, SECStatus result,
	...
	if (result == SECFailure) {
		...
		case SEC_ERROR_CERT_NOT_VALID:
			JSS_throwMsgPrErrArg(env, CERTIFICATE_NOT_YET_VALID_EXCEPTION,
					message, code);
			break;
		case SEC_ERROR_EXPIRED_CERTIFICATE:
			JSS_throwMsgPrErrArg(env, CERTIFICATE_EXPIRED_EXCEPTION, message,
					code);
			break;
		case SEC_ERROR_BAD_DER:
			JSS_throwMsgPrErrArg(env, CERTIFICATE_PARSING_EXCEPTION, message,
					code);
			break;
		case SEC_ERROR_REVOKED_CERTIFICATE:
			JSS_throwMsgPrErrArg(env, CERTIFICATE_REVOKED_EXCEPTION, message,
					code);
			break;
		default:
 ...
 ** See also: jssutil.h
 */
PRStatus JSS_clearPtrFromProxy(JNIEnv *env, jobject nativeProxy) {
	jclass proxyClass;
	...
	nativeProxyClear = (*env)->GetMethodID(env, proxyClass, "clear", "()V");
