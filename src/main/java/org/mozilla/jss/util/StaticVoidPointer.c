...
jobject JSS_PR_wrapStaticVoidPointer(JNIEnv *env, void **ref) {
	...
	finish: PR_ASSERT(refObj || (*env)->ExceptionOccurred(env));
	return refObj;
}

PRStatus JSS_PR_getStaticVoidRef(JNIEnv *env, jobject ref_proxy, void **ref) {
