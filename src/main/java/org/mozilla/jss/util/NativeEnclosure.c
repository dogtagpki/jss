...
PRStatus JSS_PR_LoadNativeEnclosure(JNIEnv *env, jobject this, jobject *ptrObj,
		jlong *ptrSize) {
	...
	field_id = (*env)->GetFieldID(env, this_class, "mPointer",
			"L" NATIVE_PROXY_CLASS_NAME ";");
	if (field_id == NULL) {
		return PR_FAILURE;
	}
...
PRStatus JSS_PR_StoreNativeEnclosure(JNIEnv *env, jobject this, jobject ptrObj,
		jlong ptrSize) {
	jclass this_class = NULL;
	jfieldID field_id = NULL;
