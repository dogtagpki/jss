...
jobject JSS_PR_wrapGlobalRef(JNIEnv *env, jobject *ref) {
	...
	finish: if (refObj == NULL && *ref != NULL) {
...
Java_org_mozilla_jss_util_GlobalRefProxy_releaseNativeResources(JNIEnv *env,
		jobject this) {
	jobject ref = NULL;
...
Java_org_mozilla_jss_util_GlobalRefProxy_refOf(JNIEnv *env, jobject clazz,
		jobject obj) {
	jobject globalRef;
