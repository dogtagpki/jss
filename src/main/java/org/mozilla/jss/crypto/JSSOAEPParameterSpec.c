...
PRStatus oaep_GetHashAlg(JNIEnv *env, jobject this, jclass this_class,
		CK_MECHANISM_TYPE *ret) {
...
PRStatus oaep_GetMGFType(JNIEnv *env, jobject this, jclass this_class,
		CK_RSA_PKCS_MGF_TYPE *ret) {
	jfieldID field_id = NULL;
...
PRStatus oaep_GetSpecifiedSourceData(JNIEnv *env, jobject this,
		jclass this_class, CK_VOID_PTR *ret, CK_ULONG *ret_len) {
	jfieldID field_id = NULL;
...
Java_org_mozilla_jss_crypto_JSSOAEPParameterSpec_acquireNativeResources(
	...
	if (oaep_GetSpecifiedSourceData(env, this, this_class, &pSourceData,
			&ulSourceDataLen) != PR_SUCCESS) {
		goto failure;
	}
	...
	if (JSS_PR_StoreNativeEnclosure(env, this, params_obj,
			sizeof(CK_RSA_PKCS_OAEP_PARAMS)) != PR_SUCCESS) {
		goto failure;
	}

	return;

	failure: free(pSourceData);
	free(oaep_params);
}
...
Java_org_mozilla_jss_crypto_JSSOAEPParameterSpec_releaseNativeResources(
	...
	if (JSS_PR_LoadNativeEnclosure(env, this, &ptr_object, &params_length)
			!= PR_SUCCESS) {
		return;
	}

	if (JSS_PR_getStaticVoidRef(env, ptr_object, (void**) &oaep_params)
			!= PR_SUCCESS || oaep_params == NULL) {
		return;
	}
	...
	if (oaep_params->ulSourceDataLen != 0 && oaep_params->pSourceData != NULL) {
		memset(oaep_params->pSourceData, 0,
				sizeof(CK_VOID_PTR) * oaep_params->ulSourceDataLen);
