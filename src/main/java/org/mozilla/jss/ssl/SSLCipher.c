...
static const CK_MECHANISM_TYPE auth_alg_defs[] = { CKM_INVALID_MECHANISM, /* ssl_auth_null */
...
/* Copied from NSS's ssl3con.c. */
static const CK_MECHANISM_TYPE kea_alg_defs[] = { CKM_INVALID_MECHANISM, /* ssl_kea_null */
...
Java_org_mozilla_jss_ssl_SSLCipher_checkSupportedStatus(JNIEnv *env,
		jclass clazz, jint cipher_suite) {
	PRInt32 allowed;
	...
	if (SSL_GetCipherSuiteInfo(cipher_suite, &info, sizeof(info))
			!= SECSuccess) {
	 ...
	 * suite). */
	if (info.authType != ssl_auth_tls13_any && info.authType != ssl_auth_null
			&& !PK11_TokenExists(auth_alg_defs[info.authType])) {
		return JNI_FALSE;
	}

	if (info.keaType != ssl_kea_null && info.keaType != ssl_kea_tls13_any
