 ...
 */
void setException(JNIEnv *env, PRFilePrivate *priv, jthrowable excep) {
...
jthrowable JSS_SSL_getException(PRFilePrivate *priv) {
...
static PRInt32 writebuf(JNIEnv *env, PRFileDesc *fd, jobject sockObj,
	 ...
	 * Write bytes
	 */
	(*env)->CallVoidMethod(env, outputStream, writeMethod, byteArray, 0,
			arrayLen);
...
static PRStatus processTimeout(JNIEnv *env, PRFileDesc *fd, jobject sockObj,
		PRIntervalTime timeout) {
	jclass socketClass;
	...
	finish: if ((*env)->ExceptionOccurred(env)) {
		return PR_FAILURE;
	} else {
		return PR_SUCCESS;
	}
...
static PRInt32 jsock_write(PRFileDesc *fd, const PRIOVec *iov, PRInt32 iov_size,
		PRIntervalTime timeout) {
	jobject sockObj;
	JNIEnv *env;
	...
	if (processTimeout(env, fd, sockObj, timeout) != PR_SUCCESS)
		goto finish;
...
static PRStatus getInetAddress(PRFileDesc *fd, PRNetAddr *addr,
	 ...
	 */
	{
		...
		getPortMethod = (*env)->GetMethodID(env, sockClass, getPortMethodName,
				GET_PORT_SIG);
...
static PRStatus jsock_getPeerName(PRFileDesc *fd, PRNetAddr *addr) {
	return getInetAddress(fd, addr, PEER_NAME);
}

static PRStatus jsock_getSockName(PRFileDesc *fd, PRNetAddr *addr) {
	return getInetAddress(fd, addr, LOCAL_NAME);
}

static PRInt32 jsock_send(PRFileDesc *fd, const void *buf, PRInt32 amount,
...
static PRInt32 jsock_recv(PRFileDesc *fd, void *buf, PRInt32 amount,
		PRIntn flags, PRIntervalTime timeout) {
	PRInt32 retval = -1;
	JNIEnv *env;
	jobject sockObj;
	...
	if (retval > 0) {
		...
		if (bytes != NULL) {
			...
			JSS_throwMsg(env, OUT_OF_MEMORY_ERROR,
...
static jboolean getBooleanProperty(JNIEnv *env, jobject sock,
...
static jint getIntProperty(JNIEnv *env, jobject sock, const char *methodName) {
	jclass sockClass;
	jmethodID method;
	jint retval = 0;
...
static void setBooleanProperty(JNIEnv *env, jobject sock,
		const char *methodName, jboolean value) {
	jclass sockClass;
	jmethodID method;
...
static void setIntProperty(JNIEnv *env, jobject sock, const char *methodName,
		jint value) {
	jclass sockClass;
	jmethodID method;
...
static void getSoLinger(JNIEnv *env, jobject sock, PRLinger *linger) {
	jint lingSecs;
...
static void setSoLinger(JNIEnv *env, jobject sock, PRLinger *linger) {
	jint lingSecs;
	jclass clazz;
	jmethodID methodID;
	jboolean onoff;
...
static PRStatus jsock_getSockOpt(PRFileDesc *fd, PRSocketOptionData *data) {
	...
	case PR_SockOpt_NoDelay:
		data->value.no_delay = getBooleanProperty(env, sockObj,
				"getTcpNoDelay");
		break;
	default:
		retval = PR_FAILURE;
...
static PRStatus jsock_setSockOpt(PRFileDesc *fd, PRSocketOptionData *data) {
	PRStatus retval = PR_SUCCESS;
	JNIEnv *env;
	jobject sockObj;
	...
	switch (data->option) {
	case PR_SockOpt_Keepalive:
		setBooleanProperty(env, sockObj, "setKeepAlive",
				data->value.keep_alive);
		break;
	case PR_SockOpt_Linger:
...
static PRStatus jsock_close(PRFileDesc *fd) {
...
static PRStatus jsock_shutdown(PRFileDesc *fd, PRShutdownHow how) {
	PRStatus retval = PR_FAILURE;
	JNIEnv *env;
	jobject sockObj;
	jmethodID methodID;
...
static const PRIOMethods jsockMethods = { PR_DESC_SOCKET_TCP,
		(PRCloseFN) jsock_close, (PRReadFN) invalidInt, (PRWriteFN) invalidInt,
		(PRAvailableFN) invalidInt, (PRAvailable64FN) invalidInt,
		(PRFsyncFN) invalidInt, (PRSeekFN) invalidInt, (PRSeek64FN) invalidInt,
		(PRFileInfoFN) invalidInt, (PRFileInfo64FN) invalidInt,
		(PRWritevFN) jsock_write, (PRConnectFN) invalidInt,
		(PRAcceptFN) invalidInt, (PRBindFN) invalidInt, (PRListenFN) invalidInt,
		(PRShutdownFN) jsock_shutdown, (PRRecvFN) jsock_recv,
		(PRSendFN) jsock_send, (PRRecvfromFN) invalidInt,
		(PRSendtoFN) invalidInt, (PRPollFN) invalidInt,
		(PRAcceptreadFN) invalidInt, (PRTransmitfileFN) invalidInt,
		(PRGetsocknameFN) jsock_getSockName,
		(PRGetpeernameFN) jsock_getPeerName, (PRReservedFN) invalidInt,
		(PRReservedFN) invalidInt, (PRGetsocketoptionFN) jsock_getSockOpt,
		(PRSetsocketoptionFN) jsock_setSockOpt, (PRSendfileFN) invalidInt,
		(PRConnectcontinueFN) invalidInt, (PRReservedFN) invalidInt,
		(PRReservedFN) invalidInt, (PRReservedFN) invalidInt,
		(PRReservedFN) invalidInt };

static void jsockDestructor(PRFileDesc *fd) {
