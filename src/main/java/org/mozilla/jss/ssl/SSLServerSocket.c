...
Java_org_mozilla_jss_ssl_SSLServerSocket_socketListen(JNIEnv *env, jobject self,
		jint backlog) {
...
Java_org_mozilla_jss_ssl_SSLServerSocket_socketAccept(JNIEnv *env, jobject self,
		jobject newSock, jint timeout, jboolean handshakeAsClient) {
	JSSL_SocketData *sock;
	PRNetAddr addr;
	...
	if (newFD == NULL) {
		...
		if (err == PR_PENDING_INTERRUPT_ERROR) {
			...
			JSSL_throwSSLSocketException(env,
					"Accept operation received IO error");
		} else {
	...
	status = SSL_HandshakeCallback(newSD->fd, JSSL_HandshakeCallback, newSD);
	if (status != SECSuccess) {
		JSSL_throwSSLSocketException(env,
...
JNIEXPORT void JNICALL
Java_org_mozilla_jss_ssl_SSLServerSocket_abortAccept(JNIEnv *env, jobject self) {
...
JNIEXPORT void JNICALL
Java_org_mozilla_jss_ssl_SSLServerSocket_clearSessionCache(JNIEnv *env,
		jclass clazz) {
	SSL_ClearSessionCache();
}

JNIEXPORT void JNICALL
Java_org_mozilla_jss_ssl_SSLServerSocket_configServerSessionIDCache(JNIEnv *env,
	...
	status = SSL_ConfigServerSessionIDCache(maxEntries, ssl2Timeout,
			ssl3Timeout, dirName);
	if (status != SECSuccess) {
		JSSL_throwSSLSocketException(env,
	...
	finish: JSS_DerefJString(env, nameString, dirName);
}
 ...
 */
JNIEXPORT void JNICALL
Java_org_mozilla_jss_ssl_SSLServerSocket_setServerCertNickname(JNIEnv *env,
...
JNIEXPORT void JNICALL
Java_org_mozilla_jss_ssl_SSLServerSocket_setServerCert(JNIEnv *env,
		jobject self, jobject certObj) {
	JSSL_SocketData *sock;
...
JNIEXPORT void JNICALL
Java_org_mozilla_jss_ssl_SSLServerSocket_setReuseAddress(JNIEnv *env,
		jobject self, jboolean reuse) {
	JSSL_SocketData *sock;
	PRStatus status;
...
Java_org_mozilla_jss_ssl_SSLServerSocket_getReuseAddress(JNIEnv *env,
		jobject self) {
	JSSL_SocketData *sock;
	PRStatus status;
	PRSocketOptionData sockOptData;
