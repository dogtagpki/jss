...
int JSSL_getOCSPPolicy() {
	...
	finish: return (int) policy;
}

static SECStatus secCmpCertChainWCANames(CERTCertificate *cert,
	...
	while (curcert) {
		...
		for (j = 0; j < caNames->nnames; j++) {
			...
			if (SECITEM_CompareItem(&issuerName, caname) == SECEqual) {
				...
				goto done;
			} else if (SECITEM_CompareItem(&compatIssuerName, caname)
					== SECEqual) {
				rv = SECSuccess;
				CERT_DestroyCertificate(curcert);
				goto done;
		...
		if ((depth <= 20)
				&& (SECITEM_CompareItem(&curcert->derIssuer,
						&curcert->derSubject) != SECEqual)) {
			oldcert = curcert;
 ...
 */
SECStatus JSSL_CallCertSelectionCallback(void *arg, PRFileDesc *fd,
	...
	clientcertselectionclass_select = (*env)->GetMethodID(env,
			clientcertselectionclass, "select",
	...
	vectorcons = (*env)->GetMethodID(env, vectorclass, "<init>", "()V");

	if (debug_cc) {
		PR_fprintf(PR_STDOUT, "  got vectorcons: %lx\n", vectorcons);
	}

	vector_add = (*env)->GetMethodID(env, vectorclass, "addElement",
			"(Ljava/lang/Object;)V");
	...
	if (names != NULL) {
		for (i = 0; i < names->numnicknames; i++) {
			...
			cert = JSS_PK11_findCertAndSlotFromNickname(names->nicknames[i],
					NULL /*pinarg*/, &slot);
			if (!cert)
				continue;
			...
			if (CERT_CheckCertValidTimes(cert, PR_Now(),
					PR_TRUE /*allowOverride*/) != secCertTimeValid) {
				if (debug_cc) {
			...
			if (rv == SECSuccess) {
				...
				privkey = PK11_FindPrivateKeyFromCert(slot, cert,
						NULL /*pinarg*/);
	...
	chosen_nickname = (*env)->CallObjectMethod(env, nicknamecallback,
			clientcertselectionclass_select, vector);
...
void JSSL_AlertReceivedCallback(const PRFileDesc *fd, void *arg,
	...
	if (socket == NULL || socket->socketObject == NULL || rc != JNI_OK
			|| env == NULL) {
		return;
	}
	...
	eventConstructor = (*env)->GetMethodID(env, eventClass, "<init>",
			"(L" SSLSOCKET_CLASS ";II)V");
	PR_ASSERT(eventConstructor != NULL);

	event = (*env)->NewObject(env, eventClass, eventConstructor,
			socket->socketObject, (int) alert->level, (int) alert->description);
	PR_ASSERT(event != NULL);

	/* socket.fireAlertReceivedEvent(event); */

	fireEvent = (*env)->GetMethodID(env, socketClass, "fireAlertReceivedEvent",
...
void JSSL_AlertSentCallback(const PRFileDesc *fd, void *arg,
		const SSLAlert *alert) {
	JSSL_SocketData *socket = (JSSL_SocketData*) arg;
	...
	eventSetDescription = (*env)->GetMethodID(env, eventClass, "setDescription",
			"(I)V");
	...
	fireEvent = (*env)->GetMethodID(env, socketClass, "fireAlertSentEvent",
			"(L" SSL_ALERT_EVENT_CLASS ";)V");
	PR_ASSERT(fireEvent != NULL);
...
void JSSL_HandshakeCallback(PRFileDesc *fd, void *arg) {
 ...
 * the socket) presents.
 */
SECStatus JSSL_DefaultCertAuthCallback(void *arg, PRFileDesc *fd,
	...
	if (peerCert) {
		if (ocspPolicy == OCSP_LEAF_AND_CHAIN_POLICY) {
			...
			rv = CERT_VerifyCertNow(CERT_GetDefaultCertDB(), peerCert, checkSig,
					certUsage, NULL /*pinarg*/);
		}
	}
...
static void addToVerifyLog(JNIEnv *env, CERTVerifyLog *log,
		CERTCertificate *cert, unsigned long error, unsigned int depth) {
 ...
 */
SECStatus JSSL_JavaCertAuthCallback(void *arg, PRFileDesc *fd, PRBool checkSig,
		PRBool isServer) {
	CERTCertificate *peerCert = NULL;
	CERTVerifyLog log;
	JNIEnv *env;
	...
	if (ocspPolicy == OCSP_LEAF_AND_CHAIN_POLICY) {
		...
		verificationResult = CERT_VerifyCert(CERT_GetDefaultCertDB(), peerCert,
				checkSig, certUsage, PR_Now(), NULL /*pinarg*/, &log);
	}
	...
	if (verificationResult == SECFailure) {
		...
		while (node) {
			...
			(*env)->CallVoidMethod(env, validityStatus, addReasonMethod, error,
					ninjacert, depth);
	 ...
	 */
	{
		...
		approvalCallbackClass = (*env)->GetObjectClass(env,
				approvalCallbackObj);
		approveMethod = (*env)->GetMethodID(env, approvalCallbackClass,
...
SECStatus JSSL_GetClientAuthData(void *arg, PRFileDesc *fd,
		CERTDistNames *caNames, CERTCertificate **pRetCert,
		SECKEYPrivateKey **pRetKey) {
	SECKEYPrivateKey *privkey;
 ...
 * certificate the peer presents.
 */
SECStatus JSSL_ConfirmExpiredPeerCert(void *arg, PRFileDesc *fd,
		PRBool checkSig, PRBool isServer) {
	SECStatus rv = SECFailure;
	SECCertUsage certUsage;
	CERTCertificate *peerCert = NULL;
	...
	if (peerCert) {
		 ...
		 */
		rv = CERT_VerifyCert(CERT_GetDefaultCertDB(), peerCert, checkSig,
				certUsage, notAfter, NULL /*pinarg*/, NULL /* log */);
	}
	if (rv != SECSuccess)
		goto finish;
