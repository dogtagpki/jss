...
Java_org_mozilla_jss_ssl_SocketBase_socketCreate(JNIEnv *env, jobject self,
	...
	if (family != SSL_AF_INET6 && family != SSL_AF_INET) {
		JSSL_throwSSLSocketException(env, "socketCreate() Invalid family!");
		goto finish;
	}
	...
	if (host != NULL) {
		...
		if (retval) {
			JSSL_throwSSLSocketException(env, "Failed to set SSL domain name");
			goto finish;
		}
	}
	...
	if (certApprovalCallback != NULL) {
		...
		sockdata->certApprovalCallback = (*env)->NewGlobalRef(env,
				certApprovalCallback);
		if (sockdata->certApprovalCallback == NULL)
			goto finish;

		/* install the Java callback */
		status = SSL_AuthCertificateHook(sockdata->fd,
				JSSL_JavaCertAuthCallback,
				(void*) sockdata->certApprovalCallback);
	} else {
		/* install the default callback */
		status = SSL_AuthCertificateHook(sockdata->fd,
				JSSL_DefaultCertAuthCallback, NULL);
	}
	if (status != SECSuccess) {
		JSSL_throwSSLSocketException(env,
	...
	if (clientCertSelectionCallback != NULL) {
		...
		sockdata->clientCertSelectionCallback = (*env)->NewGlobalRef(env,
				clientCertSelectionCallback);
		if (sockdata->clientCertSelectionCallback == NULL)
			goto finish;

		/* install the Java callback */
		status = SSL_GetClientAuthDataHook(sockdata->fd,
	...
	finish: if ((*env)->ExceptionOccurred(env) != NULL) {
		if (sockdata != NULL) {
...
JSSL_CreateSocketData(JNIEnv *env, jobject sockObj, PRFileDesc *newFD,
	...
	if (sockdata->socketObject == NULL)
		goto finish;

	/* registering alert received callback */

	status = SSL_AlertReceivedCallback(sockdata->fd, JSSL_AlertReceivedCallback,
			sockdata);

	if (status != SECSuccess) {
		JSSL_throwSSLSocketException(env,
				"Unable to install alert received callback");
		goto finish;
	}

	/* registering alert sent callback */

	status = SSL_AlertSentCallback(sockdata->fd, JSSL_AlertSentCallback,
			sockdata);

	if (status != SECSuccess) {
		JSSL_throwSSLSocketException(env,
				"Unable to install alert sent callback");
		goto finish;
	}
...
Java_org_mozilla_jss_ssl_SocketProxy_releaseNativeResources(JNIEnv *env,
		jobject this) {
 ...
 */
PRInt32 JSSL_enums[] = { SSL_ENABLE_SSL2, /* 0 *//* ssl.h */
...
JNIEXPORT void JNICALL
Java_org_mozilla_jss_ssl_SocketBase_socketBind(JNIEnv *env, jobject self,
		jbyteArray addrBA, jint port) {
	...
	if (addrBA != NULL) {
		...
		if (addrBALen != 4 && addrBALen != 16) {
			JSS_throwMsgPrErr(env, BIND_EXCEPTION, "Invalid address in bind!");
			goto finish;
		}
	...
	if (status != PR_SUCCESS) {
		JSS_throwMsgPrErr(env, BIND_EXCEPTION, "Could not bind to address");
		goto finish;
	}

	finish: JSS_DerefByteArray(env, addrBA, addrBAelems, JNI_ABORT);
}
...
Java_org_mozilla_jss_ssl_SocketBase_requestClientAuthNoExpiryCheckNative(
	...
	if (b) {
		 ...
		 */
		status = SSL_AuthCertificateHook(sock->fd, JSSL_ConfirmExpiredPeerCert,
				NULL /*cx*/);
		if (status != SECSuccess) {
			JSSL_throwSSLSocketException(env,
...
JNIEXPORT void JNICALL
Java_org_mozilla_jss_ssl_SocketBase_setSSLOption(JNIEnv *env, jobject self,
		jint option, jint on) {
	SECStatus status;
	JSSL_SocketData *sock = NULL;
...
JNIEXPORT void JNICALL
Java_org_mozilla_jss_ssl_SocketBase_setSSLOptionMode(JNIEnv *env, jobject self,
		jint option, jint mode) {
	SECStatus status;
	JSSL_SocketData *sock = NULL;
...
Java_org_mozilla_jss_ssl_SocketBase_getSSLOption(JNIEnv *env, jobject self,
		jint option) {
	JSSL_SocketData *sock = NULL;
...
PRStatus JSSL_getSockAddr(JNIEnv *env, jobject self, PRNetAddr *addr,
...
JNIEXPORT jbyteArray JNICALL
Java_org_mozilla_jss_ssl_SocketBase_getPeerAddressByteArrayNative(JNIEnv *env,
		jobject self) {
...
JNIEXPORT jbyteArray JNICALL
Java_org_mozilla_jss_ssl_SocketBase_getLocalAddressByteArrayNative(JNIEnv *env,
		jobject self) {
	jbyteArray byteArray = NULL;
	PRNetAddr addr;
	jbyte *address = NULL;
...
JNIEXPORT jint JNICALL
Java_org_mozilla_jss_ssl_SocketBase_getPeerAddressNative(JNIEnv *env,
		jobject self) {
	PRNetAddr addr;
 ...
 */
JNIEXPORT void JNICALL
Java_org_mozilla_jss_ssl_SocketBase_setClientCertNicknameNative(JNIEnv *env,
		jobject self, jstring nick) {
	PR_ASSERT(0);
	JSS_throwMsg(env, SOCKET_EXCEPTION, "JSS JAR/DLL mismatch");
}

JNIEXPORT void JNICALL
Java_org_mozilla_jss_ssl_SocketBase_setClientCert(JNIEnv *env, jobject self,
		jobject certObj) {
	JSSL_SocketData *sock = NULL;
	SECStatus status;
...
void JSS_SSL_processExceptions(JNIEnv *env, PRFilePrivate *priv) {
	...
	finish: if (currentExcep != NULL
			&& (*env)->ExceptionOccurred(env) == NULL) {
...
CERTCertificate* getRoot(CERTCertificate *cert, SECCertUsage certUsage) {
...
static SECStatus JSSL_verifyCertPKIXInternal(CERTCertificate *cert,
		SECCertificateUsage certificateUsage, secuPWData *pwdata,
		int ocspPolicy, CERTVerifyLog *log, SECCertificateUsage *usage,
