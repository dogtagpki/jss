...
static CERTCertificate* get_cert(char *host) {
	...
	for (cln = CERT_LIST_HEAD(clist); !CERT_LIST_END(cln, clist); cln =
			CERT_LIST_NEXT(cln)) {
...
static PRFileDesc* setup_nss_server(PRFileDesc *s_nspr, char *host,
		char *password, char *nickname, SECKEYPrivateKey **priv_key) {
...
bool is_finished(PRFileDesc *c_nspr, PRFileDesc *s_nspr) {
	...
	int c_sec_status;
	int s_sec_status;
	if (SSL_SecurityStatus(c_nspr, &c_sec_status, NULL, NULL, NULL, NULL, NULL)
			!= SECSuccess) {
		const PRErrorCode err = PR_GetError();
	...
	if (SSL_SecurityStatus(s_nspr, &s_sec_status, NULL, NULL, NULL, NULL, NULL)
			!= SECSuccess) {
		const PRErrorCode err = PR_GetError();
		fprintf(stderr, "error: SSL_SecurityStatus error %d: %s\n", err,
				PR_ErrorToName(err));
	...
	return c_sec_status != SSL_SECURITY_STATUS_OFF
			&& s_sec_status != SSL_SECURITY_STATUS_OFF;
}
